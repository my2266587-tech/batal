...
//
// Yemot API-module format (verified against the protocol):
//   read=<messages>=<valName>,<re_enter>,<max>,<min>,<sec>,...
// The "=" separates the spoken messages from the (comma-separated) input spec.
// For tap there is NO literal "tap" keyword — the params are positional and a
// numeric 3rd field means DTMF. Messages are joined with "." (t-/f- prefixes).
export function readTap(messages, varName, { max = 1, min = 1, sec = 7 } = {}) {
  // valName, re_enter_if_exists(empty=default), max_digits, min_digits, sec_wait
  const ops = `${varName},,${max},${min},${sec}`;
  return `read=${msgList(messages)}=${ops}`;
}

// Play message(s) and then record audio into `varName` (caller ends with #).
//
//   read=<messages>=<valName>,<re_enter>,record,<path>,<file_name>,<no_confirm_menu>
// "record" in the 3rd positional field selects recording mode. no_confirm_menu
// = "yes" skips Yemot's built-in re-record menu (we run our own confirm step);
// after the caller presses #, Yemot calls us back with the file path in valName.
export function readRecord(messages, varName) {
  const ops = `${varName},,record,,,yes`;
  return `read=${msgList(messages)}=${ops}`;
}

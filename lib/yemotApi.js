...
async function findRecordingDir() {
  for (const variant of dirVariants()) {
    ...
    if (status === "OK") return { variant, json };
  }
  return null;
}
...
export async function fetchLatestRecording() {
  const found = await findRecordingDir();
  if (!found) throw new Error("GetIVR2Dir: all path variants failed");
  const fileName = pickLatestRecording(found.json);
